...
public:
	...
	int GetState(int nID) const
		{ return (BOOL)SendMessageC(TB_GETSTATE, nID); }
...
//		{ SendMessage(TB_SETPARENT, (WPARAM)pWnd->m_hWnd); }
	...
	int  GetHotItem() const
		{ if (GetSafeHwnd()) return (int)SendMessageC(TB_GETHOTITEM); return 0; }
	...
	CSize GetPadding() const
		{ return (DWORD)SendMessageC(TB_GETPADDING); }
	CSize SetPadding(CSize sz) 
		{ return (DWORD)SendMessage(TB_SETPADDING, 0, MAKELPARAM(sz.cx,sz.cy)); }
