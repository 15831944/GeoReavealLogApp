...
CRxoScaleDlg::CRxoScaleDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CRxoScaleDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CRxoScaleDlg)
	m_bResistance = TRUE;
	m_szRMFWell = _T("");
	m_szRMFName = _T("RXO");
	//}}AFX_DATA_INIT
...
void CRxoScaleDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CRxoScaleDlg)
...
BEGIN_MESSAGE_MAP(CRxoScaleDlg, CDialog)
	//{{AFX_MSG_MAP(CRxoScaleDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CRxoScaleDlg message handlers

BOOL CRxoScaleDlg::OnInitDialog() 
{
...
void CRxoScaleDlg::OnCloseupComboRmfname() 
{
	// TODO: Add your control notification handler code here
	CString str;
...
void CRxoScaleDlg::OnBnClickedOk()
{
	OnOK();
	BOOL flag;
	if(m_DataType>2)
...
BOOL CRxoScaleDlg::CreateCrackImg()
{
	...
	strcpy(Content[2].name,szStr);
	szStr=m_szWell.Left(m_szWell.ReverseFind('.')+1)+szStr;
	CFIDIO* pArray = new CFIDIO(szStr,&Index);
	if(!pArray->Open())
	{
		AfxMessageBox("新建阵列电阻率曲线失败      ");
		delete pBut;delete pArray;
		return FALSE;
	}
	...
	if(!pRXOH->Open())
	{
		...
		delete pBut;delete pRXOH;delete pArray;
		return FALSE;
	}
	//定义进度指示器
	...
	if(Infp.Open(fn,CFile::modeReadWrite|CFile::typeBinary))
	{
		...
		if(pRxo->Open())
		{
			...
			if(bUserBreak)
			{
				...
				delete pGuage;delete pBut;delete pRXOH;delete pArray;
				return FALSE;
			}
///////////////////////////////////////////////////////////
//处理前半个窗口的数据
			...
			for(j=0;j<k12;j++)
			{
				...
				//保存电阻率阵列数据
				pArray->Write(3,m_DataNum,Value);
				pArray->Next();
			}
			//计算刻度后的图像阵列数据
			//数据范围(0-1024)
...
BOOL CRxoScaleDlg::CreateCrackImg2()
{
	int RecordNum=(int)((m_EnProDep-m_StProDep)/m_Rlev);
	BOOL bUserBreak=FALSE;
	//初始化刻度后的电阻率阵列数据
	...
	if(Infp.Open(fn,CFile::modeReadWrite|CFile::typeBinary))
	{
		...
		if(pRxo->Open())
		{
			...
			if(bUserBreak)
			{
				...
				delete pGuage;delete pBut;delete pArray;
				return FALSE;
			}
///////////////////////////////////////////////////////////
//处理前半个窗口的数据
