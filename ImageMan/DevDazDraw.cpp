...
CDevDazObj::CDevDazObj(CString szPathName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDoc)
...
void CDevDazObj::CreatePoint()
{
	...
	CString m_Path = "";
	CFIDIO::GetFullWellName(m_strPathName,m_Path.GetBuffer(512));
	m_Path.ReleaseBuffer();

	CFIDIO* pDevFIDIO = new CFIDIO(m_strPathName);
	CString StrDaz=m_DazName.Mid(m_DazName.ReverseFind('.')+1);
	StrDaz=m_strPathName.Left(m_strPathName.ReverseFind('.')+1)+StrDaz;
	CFIDIO* pDazFIDIO = new CFIDIO(StrDaz);
	if(!(pDevFIDIO->Open() && pDazFIDIO->Open()
...
BOOL CDevDazObj::Properties(CView* pView)
{
	...
	CFillListPage m_FillList;
 	if (m_ParentType != 0)
	{
		m_LineList.AddPen(&m_FramePen,"¿òÏß");
