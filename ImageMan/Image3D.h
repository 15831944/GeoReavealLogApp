...
public:
	CPlanePoint(CRect rect,float Dep,float Dip,float Dir,CString Flag,WORD PenWidth,
		COLORREF Color,float LookAngle,double HVr,BOOL bDrawBox);
	void Draw(CDC* pDC,CImageManDoc* pDoc,int StartPos);
	void DrawEllipse(CDC* pDC,CImageManDoc* pDoc,int StartPos);
	float Dir360(float dir);
...
public:
	CImage3D();
	CImage3D(CString strName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDocument);
	~CImage3D();
