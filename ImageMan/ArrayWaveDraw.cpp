...
CArrayWaveObj::CArrayWaveObj(CString szName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDoc)
...
void CArrayWaveObj::DrawHead(CDC *pDC)
{
	...
	if ((m_WaveType == 2)||(m_WaveType == 4)||(m_WaveType == 5)||(m_WaveType == 6))
	{
		...
		CRect rect0=rect;
		int i;double dx;
		dx = (m_HeadRect.Width()-4.0) / m_crNum;
...
void CArrayWaveObj::CreateWavePalette()
{
	...
	LPLOGPALETTE pLogPal = (LPLOGPALETTE) new BYTE[sizeof(LOGPALETTE)+m_crNum * sizeof(PALETTEENTRY)];
	pLogPal->palVersion = 0x300; /* bloody mysterious */
	pLogPal->palNumEntries = m_crNum;
	int i;
	if(m_WaveType == 2)
	{
		MaxClr = m_WaveColor;
		MinClr =m_WaveColor0;	
		fRed =  (GetRValue(MaxClr) - GetRValue(MinClr))/(m_crNum-1);
		fGreen = (GetGValue(MaxClr) - GetGValue(MinClr))/(m_crNum-1);
		fBlue =  (GetBValue(MaxClr) - GetBValue(MinClr))/(m_crNum-1);	
		for (i=0; i<m_crNum; i++) 
		{
	...
	else if(m_WaveType == 4)
	{
		...
		CString ColorFile=GetWorkPath("Color")+"Blaze.cip";
		CFile file(ColorFile, CFile::modeRead | CFile::shareDenyWrite );
		CArchive ar(&file, CArchive::load);	
		m_Gradient.Serialize(ar);

		RGBTRIPLE *pal = new RGBTRIPLE[m_crNum], *entry;
		m_Gradient.SetQuantization (m_crNum);
		m_Gradient.MakeEntries(pal,m_crNum);
		for(i=0; i<m_crNum; i++) 
		{
			entry = &pal[i];
...
{                     
	...
	CString WellName;
	CFIDIO::GetFullWellName(m_strPathName,WellName.GetBuffer(512));
	WellName.ReleaseBuffer();	
	float DelayValue=0.f;
	CFIDIO* DelayIO = new CFIDIO(WellName + "." + m_DelayCurve );
	if(DelayIO->Open())
	{
	...
	float* fpf = NULL;
	char* cpf = NULL;
	CFIDIO* PlusIO = new CFIDIO(WellName + "." + m_PlusCurve );
	if(PlusIO->Open())
	{
		bPlus=TRUE;
	...
	{	
		int ps=m_crNum-1;
		if(m_WaveType == 6)
			MemDC.FillSolidRect(0, Offset, rect.Width(),m_ClipRect.Height()-Offset,PALETTEINDEX(m_crNum/2));
			...
			if(y-m_Amp> rect.top)
			{
				...
				if(m_WaveType == 2)
				{
					...
					for(int xx=0;xx<k+1;xx++)
					{
						...
						{
							m_Pen.DeleteObject();
							int cr = (int) ((y-point[xx].y)*1.0/m_Amp * m_crNum);
							if (cr >ps) cr = ps;
				...
				else if(m_WaveType==4||m_WaveType==5||m_WaveType==6)
				{
     				int cr;
					for(int xx=0;xx<k+1;xx++)
					{
						cr = (int) ((m_Amp-(point[xx].y-y))*0.5f/m_Amp* m_crNum);
						if(cr<0) cr=0;
						if(cr>ps) cr=ps;
						MemDC.FillSolidRect(point[xx].x,y-m_WaveWidth-m_ClipRect.top,2,m_WaveWidth,PALETTEINDEX(cr));
					}
				}
				else
				{
