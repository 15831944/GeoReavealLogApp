...
CLayerPoint::CLayerPoint(CRect rect,CPoint pt, COLORREF ResultColor,
						 LOGFONT ResultFont,CString NoStr,bool bLit)
{
	m_NoStr=NoStr;
	m_Rect = rect;
	m_point=pt;
...
void CLayerPoint::Draw(CDC* pDC,CImageManDoc* pDoc)
{
	CRect rect=m_Rect;
	CPoint pt=m_point;
	pDoc->LPtoPP(&rect);
	...
	{
		CPen m_Pen(PS_DASHDOTDOT ,1,m_ResultColor);
		CPen* pOldPen = pDC->SelectObject(&m_Pen);
	...
	if(m_bLit)
	{
		CSize sz = pDC->GetTextExtent(m_NoStr);
		CRect rc(pt.x-sz.cx/2,pt.y-sz.cy,pt.x+sz.cx/2,pt.y);
		if(m_ResultFont.lfEscapement == 2700)
		{
			rc.left=pt.x;rc.top=pt.y-sz.cy/2;
			rc.right=pt.x+sz.cx;rc.bottom=pt.y+sz.cy/2;
		}
		pDoc->LPtoPP(&rc);
...
CTrackObj::CTrackObj(const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDocument,double sdep,double edep) 
...
{                 
	...
	m_Width = 2.0;
	m_bSelected = FALSE;
	m_TrackFramePen.lopnWidth = CPoint(2,2); m_TrackFramePen.lopnColor = RGB(0,0x80,0); m_TrackFramePen.lopnStyle = 0;
	...
	LOGFONT m_LogFont={-14,0,0,0,600,0,0,0,ANSI_CHARSET,OUT_TT_ONLY_PRECIS,
			CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,VARIABLE_PITCH | FF_SWISS,"Times New Roman"	};
    m_TrackNameFont = m_LogFont;
    m_TrackNameColor = RGB(0,0x80,0);
    m_TrackName.LoadString(IDS_TRACK);
    bDrawDepth = false;
	bDrawTitle = true;
	bTrackResize =true;
    m_DepthName.LoadString(IDS_DEPTH1);
...
{                
	...
	if(!ar.IsStoring())
	{
		...
		else
		{
			...
			LOGFONT m_LogFont={-14,0,0,0,FW_NORMAL,0,0,0,ANSI_CHARSET,OUT_TT_ONLY_PRECIS,
				CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,VARIABLE_PITCH | FF_SWISS,"Times New Roman"};
			m_DepUnitNameFont=m_LogFont;
			m_DepUnitNameColor=0x0;
			m_LayerNameFont=m_LogFont;
			m_LayerNameColor=0x0;
			m_bLayerHi=TRUE;
		}
		ar >> l; 
...
{   
	...
	m_LineList.AddPen(&m_TrackFramePen,"框线");
...
void CTrackObj::OnShowDepth(BOOL bShow)
{
	...
	if (bShow)  
	{
		...
		else 
		{
			...
			{
        	 	AfxMessageBox(IDS_TRACK_TOO_SMALL , MB_OK | MB_ICONSTOP);
				bDrawDepth = FALSE;
         		return;
 			}
...
void CTrackObj::SetChildTrackTitleHeight()
{
	...
	if (m_Dialog.DoModal() == IDOK)
	{
		...
		pDoc->UpdateAllViews(pView,HINT_UPDATE_WINDOW);
	}
}
...
void CTrackObj::CreatePoint(CDC* pDC)
{
	...
	CString str=m_LayerFileName;
	str.MakeUpper();
	int xyz=str.Find (".LIST");
	if((xyz>0)&&DetectFileExist(m_LayerFileName))
	{
		...
		int OutDepLeve=(int)(DepthToYCoor((double)m_DepthLevel)-DepthToYCoor(0.));
		...
		while(File.ReadString(buffer, 1024)) 
		{
			...
			*end = _T('\0');
			...
			{		
				BOOL chFlag=FALSE;
				//判断字符串后是否有汉字
				for(int n=0;n<szStr.GetLength();n++)
				{
					WCHAR p=szStr.GetAt(n);
					if(HIBYTE((WORD)p))
					{
						chFlag=TRUE;//有汉字
						break;
				...
				if((Flagstr.Find ("定位")==-1)&&(szStr.Find("LINE")==-1)&&(szStr!=""))
				{
					rect.top=top0;
					if(dep>m_PlotEdep&&dep-dep0>10)
					{
						...
						if(chFlag)
							SubDeviseLayer(rect,szStr,Flagstr,pDC,dep0,(float)m_PlotEdep);
						else
						{
							size00 = pDC->GetTextExtent(szStr);
							if(m_LayerNameFont.lfEscapement == 2700)
							{
								pt.x=(rect.left+rect.right-size00.cy)/2;
								pt.y=(rect.top+rect.bottom+size00.cx)/2+4;
							}
							else
							{
								pt.x=(rect.left+rect.right)/2;
								pt.y=(rect.top+rect.bottom)/2+size00.cy+4;
							}
							CLayerPoint* pObj1 = new CLayerPoint(rect,pt,m_LayerNameColor,m_LayerNameFont,szStr);
							m_LayerPoint.AddTail(pObj1);
							pt=rect.BottomRight ();
							CLayerPoint* pObj12= new CLayerPoint(rect,pt,m_LayerNameColor,m_LayerNameFont,"LINE");
					...
					if(chFlag)
						SubDeviseLayer(rect,szStr,Flagstr,pDC,dep0,dep);
					else
					{
						size00 = pDC->GetTextExtent(szStr);
						if(m_LayerNameFont.lfEscapement == 2700)
						...
						CLayerPoint* pObj2 = new CLayerPoint(rect,pt,m_LayerNameColor,m_LayerNameFont,szStr);
						m_LayerPoint.AddTail(pObj2);
						pt=rect.BottomRight ();
						CLayerPoint* pObj22= new CLayerPoint(rect,pt,m_LayerNameColor,m_LayerNameFont,"LINE");
				...
				else
				{
					...
					else
					{
						...
						if(!chFlag)
						{
							...
							if(m_LayerNameFont.lfEscapement == 2700)
							{
								pt.x=(rect0.left+rect0.right-size00.cy)/2;
								pt.y=rect0.top+size00.cx/2+4;
							}
							else
							{
								pt.x=(rect0.left+rect0.right)/2;
								pt.y=rect0.top+size00.cy+4;
							}
							CLayerPoint* pObj3 = new CLayerPoint(rect0,pt,m_LayerNameColor,m_LayerNameFont,szStr);
							m_LayerPoint.AddTail(pObj3);
						}
						else SubDeviseLayer(rect0,szStr,Flagstr,pDC,dep0,dep);
						dep0=dep;
					}
				}
			}
...
void CTrackObj::SubDeviseLayer(CRect rect,CString NoteStr,CString Flag,CDC* pDC,float dep0,float dep)
{
	CRect rect0=rect;
	CPoint pt,pt0;
	CFont m_Font,m_Font2700;
	LOGFONT layerFont=m_LayerNameFont;
	LOGFONT LogFont2700=layerFont;
	CSize size0;
	size0.cx = layerFont.lfWidth; 
	size0.cy =layerFont.lfHeight ;
	CSize size1 = size0;
	...
	if(layerFont.lfEscapement == 2700)
	{
		layerFont=LogFont2700;
		m_Font.CreateFontIndirect(&layerFont);
		layerFont.lfHeight = size1.cy;
		layerFont.lfWidth = size1.cx;
	}	
	else m_Font.CreateFontIndirect(&layerFont);
	
	CFont* pOldFont;
	CSize size,size00;
	if((NoteStr.Find("(未完)")>-1)
		||(NoteStr.Find("（未完)")>-1)
		||(NoteStr.Find("(未完）")>-1)
		||(NoteStr.Find("（未完）")>-1))
	{
		...
		CLayerPoint* pObj3 = new CLayerPoint(rect1,pt,m_LayerNameColor,layerFont,"未");
		m_LayerPoint.AddTail(pObj3);
		if (layerFont.lfEscapement == 2700)
		{
			pt.y=rect.bottom +size.cy/2;
		...
		CLayerPoint* pObj4 = new CLayerPoint(rect1,pt,m_LayerNameColor,layerFont,"完");
	...
	if (layerFont.lfEscapement == 2700)
		...
		pt.x = (rect.left+rect.right) / 2;
	int AliasLen=NoteStr.GetLength ();
	...
	if(Flag.Find ("垂直定位")>-1)
	{
		...
		{			
			...
			CLayerPoint* pObj5 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
	...
	else if(Flag.Find ("水平定位")>-1)
	{
		...
		int ll=NoteStr.GetLength()-2;
		for(i=0;i<StrNum;i++)
		{			
			if(layerFont.lfEscapement == 2700)
				Str=NoteStr.Mid (ll-i*2,2);
			else
				Str=NoteStr.Mid (i*2,2);
			rect.top =pt.y-size.cy;
			rect.bottom =pt.y;
			CLayerPoint* pObj6 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
	...
	else if(Flag.Find ("顶底标识")>-1)
	{
		if(!m_bLayerHi)
		{
			size00 = pDC->GetTextExtent(NoteStr);
			if(layerFont.lfEscapement == 2700)
				pt.x=(rect.left+rect.right-size00.cy)/2;
			else
				pt.x=(rect.left+rect.right)/2;
			...
			CLayerPoint* pObj44 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,NoteStr);
			...
			CLayerPoint* pObj55 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,NoteStr);
			m_LayerPoint.AddTail(pObj55);
			i=AliasLen;
		}
		else
		{
			Str=NoteStr.Left(StrNum*2);
			i=StrNum;
			...
			CLayerPoint* pObj66 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
			...
			if(i*2<AliasLen)
			{
				...
				if(Str1.GetLength()>1)
				{
					...
					CLayerPoint* pObj222 = new CLayerPoint(rect,pt,m_LayerNameColor,LogScript0,Str);
					m_LayerPoint.AddTail(pObj222);
				}
			}
			//底部层位标注	
			Str=NoteStr.Left(StrNum*2);
			...
			CLayerPoint* pObj77 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
	...
	{		
		if(SpaceNum<StrNum)
		{
			if(StrNum*size.cy>rect.Height())
			{
				...
				{			
					...
					CLayerPoint* pObj7 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
			...
			else if(SpaceNum<1)
			{
				...
				pt.y=(rect.top+rect.bottom)/2;
				bool bLit=false;
				if(pt.y>yx0-40&&pt.y<yx0+40)
				{
					if((rect.Height()-90)<size.cy*StrNum)
						bLit=true;
					else
					{
						if(pt.y>yx0)
				...
				{		
					...
					rect.top =pt.y-size.cy;
					rect.bottom =pt.y;
					CLayerPoint* pObj7 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str,bLit);
					m_LayerPoint.AddTail(pObj7);
					pt.y+=size.cy;
					if(!bLit&&pt.y>yx0-40&&pt.y<yx0+40)
						pt.y=yx0+50;
				}
				if(i>0) 
					pt.y-=size.cy+3;
			...
			{
				int yDlta=(SpaceNum-1)/2*OutDepLeve;
				if(yDlta<0) yDlta=0;
				pt.y=DepTop+yDlta+(OutDepLeve-StrNum*size.cy)/2+size.cy;
				if(layerFont.lfEscapement == 2700)
					pt.y-=size.cy/2;
				for(i=0;i<StrNum;i++)
				{			
					...
					rect.top =pt.y-size.cy;
					rect.bottom =pt.y;
					CLayerPoint* pObj7 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str,true);
					m_LayerPoint.AddTail(pObj7);
					pt.y+=size.cy;
				}
				if(i>0) pt.y-=size.cy+3;
		...
		{						
			for(i=0;i<StrNum;i++)
			{
				...
				rect.top =pt.y-size.cy;
				rect.bottom =pt.y;
				CLayerPoint* pObj10 = new CLayerPoint(rect,pt,m_LayerNameColor,layerFont,Str);
	...
	if(i*2<AliasLen)
	{
		...
		if(Str1.GetLength()>1)
		{
			...
			CLayerPoint* pObj111 = new CLayerPoint(rect,pt,m_LayerNameColor,LogScript,Str);
			m_LayerPoint.AddTail(pObj111);
		}
	}
	pDC->SelectObject(pOldFont);
	if(Flag.Find ("定位")==-1)
...
HCURSOR CTrackObj::GetHandleCursor(int nHandle)
{
	...
	{
    case 0:
		if ((bDrawTitle) || (m_SelectMode == move)) return AfxGetApp()->LoadCursor(IDC_SELECT);
		else return AfxGetApp()->LoadCursor(IDC_TRACK_SELECT);
	case 6:
...
void CTrackObj::InitStatusBar()
{
	CDrawObj::InitStatusBar();
	if (m_StatusBar.m_hWnd == NULL) return;
	CString m_AliasName;
	if (m_TrackName != "") m_AliasName = "Track";
	else m_AliasName = m_TrackName;
	CString szHT, szHT1;
	...
	case 5:
		{
			if (nDragHandle == 5) 
			{
				szHT1.LoadString(IDS_DEPTHLEFTBOUNDARY);
				m_StatusBar.SetPaneText(0,szHT+m_AliasName+szHT1);
			}
			else
			{
				szHT1.LoadString(IDS_DEPTHRIGHTBOUNDARY);
				m_StatusBar.SetPaneText(0,szHT+m_AliasName+szHT1);
			}
		} 
		break;
	...
	case 6:
		{
			if (nDragHandle == 3) 
			{
				szHT1.LoadString(IDS_TITLEHEIGHT);
				m_StatusBar.SetPaneText(0,szHT+m_AliasName+szHT1);
			}
			else 
			{
	...
	case 5:
		{
			...
			m_StatusBar.SetPaneText(2,text);
			text.LoadString(IDS_LOCATION1);
			if ((nDragHandle == 2) || (nDragHandle == 5)) sprintf(strX,"%7.2f",rect.left*0.01);
...
void CTrackObj::DrawFrame(CView* pView, CDC* pDC)
{
	...
	{
		m_Pen.CreatePen(m_TrackFramePen.lopnStyle,3,m_TrackFramePen.lopnColor);
 	...
 	if (m_TrackClientBrush.lbStyle != BS_NULL) 
 	{
		if (m_TrackClientBrush.lbStyle == BS_OWNER)
		{
			if (m_TrackClientBrush.lbHatch > 0) 
				LithFillBox(m_ClipRect,m_TrackClientBrush.lbHatch,m_TrackClientBrush.lbColor,m_ClientRect.TopLeft(),pDC);
		}
		else
		{
			CBrush br;
		 	if (m_TrackClientBrush.lbStyle == BS_HATCHED) 
			 	br.CreateHatchBrush(m_TrackClientBrush.lbHatch,m_TrackClientBrush.lbColor);
	...
	m_ClipRect &= rect;
 	if (m_TrackHeadBrush.lbStyle != BS_NULL) 
 	{
		if (m_TrackHeadBrush.lbStyle == BS_OWNER)
		{
			...
			LithFillBox(m_ClipRect,m_TrackClientBrush.lbHatch,m_TrackClientBrush.lbColor,m_HeadRect.TopLeft(),pDC);
		}
		else
		{
			CBrush br;
		 	if (m_TrackHeadBrush.lbStyle == BS_HATCHED) 
			 	br.CreateHatchBrush(m_TrackHeadBrush.lbHatch,m_TrackHeadBrush.lbColor);
...
void CTrackObj::DrawDepth(CView* pView, CDC* pDC)
{
 	...
 	if (m_TrackDepthBrush.lbStyle != BS_NULL) 
 	{
		if (m_TrackDepthBrush.lbStyle == BS_OWNER)
		{
			if (m_TrackDepthBrush.lbHatch > 0) 
				LithFillBox(m_ClipRect,m_TrackClientBrush.lbHatch,m_TrackClientBrush.lbColor,m_DepthRect.TopLeft(),pDC);
		}
		else
		{
		 	CBrush br;
		 	if (m_TrackDepthBrush.lbStyle == BS_HATCHED) 
			 	br.CreateHatchBrush(m_TrackDepthBrush.lbHatch,m_TrackDepthBrush.lbColor);
	...
	{
		size.cx = m_DepthNameFont.lfHeight; size.cy = m_DepthNameFont.lfWidth;
		CSize size1 = size;
		pDC->LPtoDP(&size);
		...
		(m_DepthPosition!=IDM_NONE))
	{
		...
		if(m_DepUnitNameFont.lfEscapement == 2700)
			pDC->TextOut((rect.left+rect.right)/2-size.cy/2, rect1.top-size.cx/2-3,m_DepUnitName);
		else
	...
	m_Depth = m_Sdep;
	while (m_Depth <= m_Edep)
	{
		y = DepthToYCoor((double)m_Depth);
		y -= pDoc->m_WinOrg.y;
		sprintf(ss,"%4i",m_Depth);
		if (m_DepthValueFont.lfEscapement == 2700)
		{
			...
			case IDM_CENTER: 
				{
					...
					pDC->DPtoLP(&pt,1);
				    pDC->SetTextAlign(TA_CENTER | TA_BOTTOM);  
					pDC->TextOut(pt.x-size.cy/2,pt.y,ss,4);
					pDC->RestoreDC(nSaveDC);
				}break;
 			case IDM_LEFT: 
	 			{
		...
		else
		{
			...
			case IDM_CENTER: 
				{
				    pDC->SetTextAlign(TA_CENTER | TA_BOTTOM);  
					pDC->TextOut((rect.left+rect.right)/2,y+size.cy/2,ss,4);
				}break;
 			case IDM_LEFT: 
	 			{
				    pDC->SetTextAlign(TA_LEFT | TA_BOTTOM);  
 					pDC->TextOut(rect.left+2,y+size.cy/2,ss,4);
 				}break;
			case IDM_RIGHT: 
				{
				    pDC->SetTextAlign(TA_RIGHT | TA_BOTTOM);  
					pDC->TextOut(rect.right-2,y+size.cy/2,ss,4);
				}break;
 	...
 	if (m_TrackTitleBrush.lbStyle != BS_NULL) 
 	{
		if (m_TrackTitleBrush.lbStyle == BS_OWNER)
		{
			if (m_TrackTitleBrush.lbHatch > 0) 
				LithFillBox(m_ClipRect,m_TrackClientBrush.lbHatch,m_TrackClientBrush.lbColor,m_TitleRect.TopLeft(),pDC);
		}
		else
		{
			CBrush br;
		 	if (m_TrackTitleBrush.lbStyle == BS_HATCHED) 
			 	br.CreateHatchBrush(m_TrackTitleBrush.lbHatch,m_TrackTitleBrush.lbColor);
...
void CTrackObj::OnLButtonDown(CView* pView, UINT nFlags, const CPoint& point)
{
	...
	if ((m_SelectMode == size) || (m_SelectMode == move)) CreateFocusObj(pView);
	if ((m_bSelected) && (m_pFocusObj != NULL)) m_pFocusObj->InitStatusBar();
}
...
void CTrackObj::OnLButtonUp(CView* pView, UINT nFlags, const CPoint& point)
{
	...
	if (m_bSelected)
	{
		if (m_pFocusObj != NULL) m_pFocusObj->ReleaseStatusBar();
		if (c_last == c_down) 
		{
			DeleteFocusObj(pView);
		}
...
BOOL CTrackObj::MatchWellName(CString strName, CString NewstrPathName)
{
	...
	while (pos != NULL)
	{
		...
		if((pObj != NULL)&& (pObj->IsKindOf(RUNTIME_CLASS(CFIDDrawObj))))
		{
			CFIDIO::GetFIDName(pObj->m_strPathName,szFIDName);
			CurveName=szFIDName;			
			if(CurveName == strName) 
			{
				if(pObj->IsKindOf(RUNTIME_CLASS(CImage3D)))
					((CImage3D *)pObj)->m_PlaneName=NewstrPathName.Left(NewstrPathName.ReverseFind('\\')+1)+"HandWork.List";
				pObj->m_strPathName=NewstrPathName;
				pObj->m_bFlush = TRUE;
				pObj->m_bErrorFID = FALSE;
				flag=TRUE;
			}
