...
BOOL CInPlaceFrame::OnCreateControlBars(CFrameWnd* pWndFrame, CFrameWnd* pWndDoc)
{
	...
	m_wndPlotBar.SetOwner(this);
	// Create toolbar on client's frame window
	if (!m_wndToolBar.Create(pWndFrame) ||
		!m_wndToolBar.LoadToolBar(IDR_LOGPLTTYPE_SRVR_IP))
	{
		TRACE0("Failed to create toolbar\n");
		return FALSE;
	}
	if (!m_wndPlotBar.Create(pWndFrame) ||
	...
	// TODO: Remove this if you don't want tool tips or a resizeable toolbar
	m_wndPlotBar.SetBarStyle(m_wndPlotBar.GetBarStyle() |
		CBRS_TOOLTIPS | CBRS_FLYBY | CBRS_SIZE_DYNAMIC);
	...
	pWndFrame->EnableDocking(CBRS_ALIGN_ANY);
	pWndFrame->DockControlBar(&m_wndPlotBar);
