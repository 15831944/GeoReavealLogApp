	...
	{	
		...
		else
		{
			...
			if(y-pt.y>1)
			{
				...
				m_pPens = new Pen(CurveColor,(float)pPen->GetWidth());
				pic->DrawLine(m_pPens,pt.x,pt.y,x,y);
...
CCurveObj::CCurveObj(CString szPathName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDoc)
...
HCURSOR CCurveObj::GetHandleCursor(int nHandle)
{
	if (nHandle < 9) return CFIDDrawObj::GetHandleCursor(nHandle);
	else
	{
		CImageManApp* pApp=(CImageManApp*)AfxGetApp();
		if(pApp->m_Nib )
			return AfxGetApp()->LoadCursor(IDC_CURSOR_BOX);
		else
			return AfxGetApp()->LoadCursor(IDC_CURSOR_CROSS);
	}
}
/*
void CCurveObj::CreatScriptPoint()
{
	POSITION pos = m_ScriptPoint.GetHeadPosition();
	while (pos != NULL) 
		delete m_ScriptPoint.GetNext(pos);
	m_ScriptPoint.RemoveAll();
	if(m_Factor==4)
	{
		...
		if(pFIDIO->Open())
		{
			...
			while(idep<m_PlotEdep) 
			{
				...
				if(Value!=-999.25f&&Value!=-9999.f&&Value!=-99999.f)
				{
					...
					CScriptPoint* pObj=NULL;
					pObj = new CScriptPoint(point,(float)idep,content,m_ScriptColor,m_ScriptFont);
					m_ScriptPoint.AddTail(pObj);
				}
				idep+=irlev;
			}
		}
	...
	if(pFIDIO->Open() && IsDepthUnitOK(pFIDIO))
	{
		...
		if(sdep<m_PlotSdep) 
			sdep=(float)m_PlotSdep;
		edep=pFIDIO->GetEndDepth();

		if(edep>m_PlotEdep) 
			edep=(float)m_PlotEdep;
		...
		if(rlev>0.001)
		{
			...
			Num = pFIDIO->Read(2,(unsigned int )Num,Val);

			// 打开文件，读取接箍深度、长度及对应的曲线值
			// 曲线值用于计算标注的X位置
			TCHAR buffer[1024];
			TCHAR *token, *end;
			CStdioFile File;
			if(File.Open(m_ScriptFileName, CFile::modeRead | CFile::typeText))
			{
				File.ReadString(buffer, 1024);	// 读文件列标题
				File.ReadString(buffer, 1024);	// 读文件列单位
				CString str,szStr;
				int i,j;
				float dep,value;
				BOOL bDraw;
				...
				while(File.ReadString(buffer, 1024)) 
				{
					...
					*end = _T('\0');
					...
					if(dep>sdep&&dep<edep&&!szStr.IsEmpty())
					{
						pt.y= DepthToYCoor(dep);	
						j=(dep-sdep)/(edep-sdep)*Num;
						if(j>Num-1) j=Num-1;
						value=Val[j];
						if(m_CalType == 1) 
						{
							if(value< 0.0001f) 
								value= 0.0001f;
							pt.x=m_Rect.left + Round((log10(value)-log10(m_LeftVal))*tmp);
						}
						else
							pt.x=m_Rect.left + Round((value - m_LeftVal)*tmp);

						CScriptPoint* pObj=NULL;
						pObj = new CScriptPoint(pt,dep,szStr,m_ScriptColor,m_ScriptFont);
						m_ScriptPoint.AddTail(pObj);
					}
				}
				File.Close();
...
void CCurveObj::DrawHead(CDC *pDC)
{
	...
	if ((k > 0) && (m_bHeadShowFill))
	{
		...
		if (!(pFill->m_Text.IsEmpty()))
		{
			...
			pDC->TextOut(rect.left + 4, rect.bottom-4, pFill->m_Text);
		}		
...
int CCurveObj::GetPoint(double& sdep,double& edep, CPoint*& pt)
{
	...
	if (m_bFlush || (sdep+eps <m_SDep) || (edep-eps > m_EDep))
	{
	...
	pt = m_Point + k;		
	if (num+k > m_PointNum) num = m_PointNum-k;
	return num;
}
...
int CCurveObj::GetValue(double& sdep,double& edep, float*& value)
{
	...
	if (m_bFlush)
	{
		m_bErrorFID = TRUE;
		CFIDIO m_FIDIO(m_strPathName);
		if (m_FIDIO.Open() && IsDepthUnitOK(&m_FIDIO))
		{
			m_Rlev = m_FIDIO.GetDepthLevel();
			if (m_Rlev > 0.001) 
			{
...
int CCurveObj::CreatePoint(double& sdep, double& edep)
{
	...
	{
		m_Rlev = pFIDIO->GetDepthLevel();
		if (m_Rlev > 0.001) m_bErrorFID = FALSE;
	}
	if (m_bErrorFID) 
	{
		m_SDep = m_EDep = m_SSdep = m_EEdep = 0;
	...
	if (m_PointNum > 0)
	{
		...
		for(i=0;i<m_FilterNum;i++)
		{
			...
			{
				if((m_Value[n-1]!=-9999.f)&&(m_Value[n]!=-9999.f)&&(m_Value[n+1]!=-9999.f)
					&&(m_Value[n-1]!=-99999.f)&&(m_Value[n]!=-99999.f)&&(m_Value[n+1]!=-99999.f)
					&&(m_Value[n-1]!=-999.25f)&&(m_Value[n]!=-999.25f)&&(m_Value[n+1]!=-999.25f))	
				{
...
{
	char ext[128]; 
	CFIDIO::GetFIDName(m_strPathName,ext); 
	int v = atoi(ext);
	if (v>100) v = 100;
	if (v < 0) v = 0;
...
void CCurveObj::DrawFill(CView* pView, CDC* pDC)
{
	...
	{
		CFIDIO m_FID2(pFill->pSourceCurve->m_strPathName);
		if (m_FID2.Open()&& IsDepthUnitOK(&m_FID2))
		{
	...
	{ 
		...
		{
			if(point[i].x<m_Rect.left) 
				point[i].x=m_Rect.left;
			if(point[i].x>m_Rect.right) 
				point[i].x=m_Rect.right;
		}
		if (pFill->m_TargetCurveName == "左边界")
		{
			if(pDoc->m_FillLithGround&&(pFill->m_LthNo!=1))
				FillCurve(m_Rect.left,point,PointNum,pFill->m_LthNo,m_CurveColor,BrushOrg,pDC,FILLLEFTRIGHT,pFill->m_Color);
			else
		...
		{
			if(pDoc->m_FillLithGround&&(pFill->m_LthNo!=1))
				FillCurve(m_Rect.right,point,PointNum,pFill->m_LthNo,m_CurveColor,BrushOrg,pDC,FILLLEFTRIGHT,pFill->m_Color);
			else
		...
		else 
		{
			...
			else
			{
				...
				if(PointNum1>1)
				{
					pFill->pTargetCurve->pDoc->LPtoPP(point1,PointNum1);
					CPoint* point2=new CPoint[PointNum];
					for(i=0;i<PointNum;i++)
					{
						...
						if(point2[i].x<m_Rect.left) 
							point2[i].x=m_Rect.left;
						if(point2[i].x>m_Rect.right) 
							point2[i].x=m_Rect.right;
					}
					if(pDoc->m_FillLithGround&&(pFill->m_LthNo!=1))
						FillCurve(point,point2,PointNum,pFill->m_LthNo,m_CurveColor,BrushOrg,pDC,pFill->m_FillMode,pFill->m_Color);
					else
...
{        
	...
	if (IsGridCurve())
	{
		...
		m_LinePen.org = 0;
		m_Line.pDC = pDC;
		if(pDoc->m_ScaleFactor!=100||pDoc->m_ScaleNo==0)
			DLine(x,m_ClipRect.top,x,m_ClipRect.bottom,&m_Line);
		else
			AntiDLine(x,m_ClipRect.top,x,m_ClipRect.bottom,&m_Line);
		return;
	}
	double sdep;
	double edep;
		...
		{			
			if(PN==8)
			{
				...
				{				
					...
					pDC->Ellipse (point[i].x-2,point[i].y-2,point[i].x+2,point[i].y+2);
				}
			}
			else if(PN==7)
			{
				for (i=0; i<PointNum; i++)
				{
					...
					pDC->LineTo(point[i].x-2,point[i].y);
					pDC->Ellipse (point[i].x-2,point[i].y-2,point[i].x+2,point[i].y+2);
				}
			}
			else
		...
		else
		{
			...
			else
			{
				if(Factor>1)
				{
					flag=1;
					for (i=1; i<PointNum; i++)
						AntiDLine(point[i-1].x-ww,point[i-1].y,point[i].x-ww,point[i].y,&m_Line);
					flag=1;
					for (i=1; i<PointNum; i++)
						AntiDLine(point[i-1].x+ww,point[i-1].y,point[i].x+ww,point[i].y,&m_Line);
				}
				flag=1;
				for(i=1; i<PointNum; i++)
					AntiDLine(point[i-1].x,point[i-1].y,point[i].x,point[i].y,&m_Line);
			}
			if (flag == 0) 
				pDC->LineTo(pt[0]);			
			if (point2!= NULL)
			{
				...
				else
				{
					if(Factor>1)
					{
						flag=1;
						for (i=1; i<PointNum; i++)
							AntiDLine(point2[i-1].x-ww,point2[i-1].y,point2[i].x-ww,point2[i].y,&m_Line);
						flag=1;
						for (i=1; i<PointNum; i++)
							AntiDLine(point2[i-1].x+ww,point2[i-1].y,point2[i].x+ww,point2[i].y,&m_Line);
					}
					...
					for (i=1; i<PointNum; i++)
					{
						AntiDLine(point2[i-1].x,point2[i-1].y,point2[i].x,point2[i].y,&m_Line);
					}
				}
				if (flag == 0) 
					pDC->LineTo(pt[0]);				
...
void CCurveObj::FindAllFillCurve()
{
	...
	{
		for (int i=m_FillList.GetSize()-1; i>=0; i--)
		{
...
void CCurveObj::OnLButtonDown(CView* pView, UINT nFlags, const CPoint& point)
{
	...
	m_HeadRect0=m_HeadRect;m_Rect0=m_Rect;
    if (m_bSelected) 
	{
		...
		if (!pDoc->m_MouseTip) return;
	    CDrawObj::OnLButtonDown(pView,nFlags,point);
	 	InitStatusBar();
		m_SelectMode = editdata;
		DrawFocus(pView);
	}
...
{	
	...
	{
		m_MovePointX=point.x-m_MovePointX;
		m_MovePointY=point.y-m_MovePointY;	
		m_HeadRect=m_HeadRect0;m_Rect=m_Rect0;
		m_bSelected=FALSE;
    ...
    c_ToolShape = nonetool;
    m_SelectMode = none;
	if (c_last.x == c_down.x) return;
	double sdep; 
	sdep = YCoorToDepth(pt[1].y + pDoc->m_WinOrg.y);
	CFIDIO* pFID = new CFIDIO(m_strPathName);
	if (pFID->Open())
	{
		pFID->SeekToDepth((float)(sdep));
		pFID->Write(2,1,&value[1]);
		pFID->Close();
		m_bFlush = TRUE;
		CRect rect = m_Rect;
		rect.top = pt[0].y; rect.bottom = pt[2].y;
		CClientDC dc(pView);
		...
		pView->UpdateWindow();
		((CMainFrame*)AfxGetMainWnd())->OnWellDataUpdate(0,0L);
	}
	delete pFID;
}
...
int CCurveObj::HitTest(CView* pView,const CPoint& point)
{
	...
	if (pFID->Open())
	{
		...
		{ 
			...
			{
				if (i == 1) continue;
				while (pt[i].x < m_Rect.left) pt[i].x = m_Rect.right - (m_Rect.left - pt[i].x) / m_Factor;
				while (pt[i].x > m_Rect.right) pt[i].x = m_Rect.left + (pt[i].x - m_Rect.right) / m_Factor;
			}
			else if (m_Factor == 0) 
			{
		...
		if ((point.x < pt[1].x+2) && (point.x >= pt[1].x-2)) 
		{
			CImageManApp* pApp=(CImageManApp*) AfxGetApp();
			if(pApp->m_MoveDep||pApp->m_PlusValue)	nHandle = 0;
			else									nHandle = 9;
		}
	}
	delete pFID;
...
BOOL CCurveObj::OnToolTip(CView* pView,CRect& rect,CString& text)
{
	...
	if (nHandle == 9)
	{
		rect.InflateRect(2,2);
		char str[200];
		m_PointDep = YCoorToDepth(point.y);
		m_PointDep=((int)(m_PointDep/m_Rlev))*m_Rlev;
		dtos(m_PointDep,str);
...
void CCurveObj::InitStatusBar()
{
	CFIDDrawObj::InitStatusBar();
	if (m_StatusBar.m_hWnd == NULL) return;
	if (nDragHandle == 9) 
	{
		CString szHT, szHT1;
		szHT.LoadString(IDS_DRAGMOUSE);
		szHT1.LoadString(IDS_VALUE);
		if (pDoc->m_MouseTip) 
			m_StatusBar.SetPaneText(0,szHT+m_AliasName+szHT1);
		else m_StatusBar.SetPaneText(0,"");
		double dep;
		dep = YCoorToDepth(pt[1].y + pDoc->m_WinOrg.y);
		char s[10];
		sprintf(s,"%8.3f",dep);
		CString text;
		text.LoadString(IDS_DEPTH);
		text = text + s;
...
void CCurveObj::ShowStatus()
{
	if (m_StatusBar.m_hWnd == NULL) return;
	if (nDragHandle < 9) CFIDDrawObj::ShowStatus();
	else
	{
		char s[20];
		value[1] = (float)(m_LeftVal1 + (m_RightVal1 - m_LeftVal1) * (pt[1].x - m_Rect.left) / m_Rect.Width());
		if (m_CalType == 1) 
		{
			value[1] = (float) pow(10.,value[1]*1.); 
		}
		dtos((double)value[1],s);
		CString text;
		text.LoadString(IDS_VALUE);
		text += s;
...
{
		if (IsGridCurve()) m_bErrorFID = FALSE;	
		CRect rect = m_Rect;
		pDoc->LPtoPP(&rect);
		CDC *pDC=pView->GetDC ();
