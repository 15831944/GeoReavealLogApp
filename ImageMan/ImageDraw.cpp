...
void CImageObj::Serialize( CArchive& ar )
{
	...
	if(!ar.IsStoring())
	{
		...
		ar >> m_CtrlVer;					//绘图对像版本号
		ar >> m_ColorGrade;					//色标灰度级
		ar >> m_CrMode;						
		if(m_CtrlVer>2)						
		{									
			ar >> m_LeftVal;				//低刻度
		...
		ar >> m_BkColor;                   //盲区填充色
		ar >> m_bColor;                    //是否电导率成像、FALSE色标倒置
		ar >> m_Bit;                       //钻头直径
		ar >> m_HorScale;                  //横向刻度
		SerializeLogFont(ar,m_ScaleFont);
		if(m_CtrlVer>1)
			ar >> m_SchemeFile;            //色标文件名
		else
			m_ColorGrade=32;
		CreateImagePalette();		
	}
...
void CImageObj::DrawHead(CDC *pDC)
{
	...
	CSize size = pDC->GetTextExtent("国");
		...
		||m_strPathName.Find("IMAGE")>0)
	{
		...
		//横向比例
		CString szStr;
		szStr.Format("横向比例(1:%d)",m_HorScale);
