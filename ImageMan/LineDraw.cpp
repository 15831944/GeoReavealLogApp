...
{
 	m_DrawShape = lineobj;
	m_LogBrush.lbColor = 0x0;
}

CLineObj::CLineObj(const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDocument)
...
void CLineObj::Draw(CView* pView,CDC* pDC)
{
	...
	pDoc->LPtoPP(&pt1);
	pDoc->LPtoPP(&pt2);
	CPen pen(0,1,0x0101FF);
	CPen *OldPen=(CPen *)pDC->SelectObject(&pen);
	CBrush *OldBrush=(CBrush *)pDC->SelectStockObject(NULL_BRUSH);
	pDC->MoveTo(pt1);pDC->LineTo(pt2);
	CRect rect(pt1.x-4,pt1.y-4,pt1.x+4,pt1.y+4);
	pDC->Ellipse(rect);
	pDC->SelectObject(OldPen);
	pDC->SelectObject(OldBrush);
}

int CLineObj::HitTest(CView* pView,const CPoint& point)
{
	ASSERT_VALID(this);
	if (m_bSelected)
	{
