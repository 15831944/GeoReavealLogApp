...
CDepSect::CDepSect(const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDocument)
...
void CDepSect::Draw(CView* pView,CDC* pDC)
{
	...
	pDC->MoveTo (HalfX-3,point1.y-1);pDC->LineTo (HalfX+3,point1.y-1);
	pDC->MoveTo (HalfX-3,point1.y+1);pDC->LineTo (HalfX+3,point1.y+1);
	pDC->MoveTo (HalfX-3,point2.y-1);pDC->LineTo (HalfX+3,point2.y-1);
	pDC->MoveTo (HalfX-3,point2.y+1);pDC->LineTo (HalfX+3,point2.y+1);
...
void CDepSect::OnMouseMove(CView* pView, UINT nFlags, const CPoint& point)
{
	...
	if (m_SelectMode == move)
	{
		...
		while (pos != NULL)
		{
			CDrawObj* pObj = (CDrawObj*)m_ObjList.GetNext(pos);
			if (pObj != NULL) 
				pObj->OnMouseMove(pView, nFlags, point1);
		}
