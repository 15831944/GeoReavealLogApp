...
CLthNotePoint::CLthNotePoint(CRect rect, COLORREF LthNoteColor,LOGFONT LthNoteFont,CString NoteStr,CImageManDoc* pDocument)
{
	pDoc = pDocument;
	m_NoteStr=NoteStr;
	m_Rect = rect;
...
CLthNoteObj::CLthNoteObj(CString strPathName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDoc)
		:CFIDDrawObj(strPathName,rect,pParentObj,pDoc)
{
	m_DrawDirect = VERDIR;
	m_DrawShape = lthnoteobj;
	m_AliasName = "岩性描述";
	strcpy(m_LthNoteFont.lfFaceName,"Times New Roman"); 
	m_LthNoteFont=m_NameFont;
	m_LthNoteFont.lfHeight = -14;
	m_LthNoteColor=RGB(0,0,0);
