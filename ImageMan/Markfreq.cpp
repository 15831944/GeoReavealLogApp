...
CMarkFreqObj::CMarkFreqObj(CString szPathName, const CRect& rect,CDrawObj* pParentObj,CImageManDoc* pDoc)
			:CFIDDrawObj(szPathName,rect,pParentObj,pDoc)
{
	m_DrawShape = Markfreqobj;
	m_bDrawDipFreq=FALSE;
	m_AliasName.LoadString(IDS_FREQUENCYSTATISTIC);
		...
		try 
		{
			...
			while (File.ReadString(buffer, 1024)) 
			{
				...
				*end = _T('\0');
				...
				if(Dep>m_PlotEdep)
					continue;
				if((Dip0>0.005f)&&(Dir0>0.05f)&&
					((Flag=="层理")))
					//||(Flag=="充填缝")||(Flag=="张开缝")||(Flag=="压裂缝")||(Flag=="半充填缝")))
				{						
...
{    
	...
	int m_BottomDepth =m_TopDepth+m_DepLevel;
	m_SDep = m_TopDepth; m_EDep = m_BottomDepth;
	...
	{                
		if (m_DangRect.bottom > m_ClipRect.top)
		{
			...
			while (pos != NULL)
			{
				...
				dang=pObj->m_Dip;
				ddir=pObj->m_Dir;
				if((dang==-9999.f)||(dang==-999.25f)) continue;
				if((ddir==-9999.f)||(ddir==-999.25f)) continue;
				if (!((dang <= m_RightVal) && (dang >= m_LeftVal))) 
					continue;
				i = 0;
				while ((ddir >= i*m_AzimLevel) && (i < m_AzimNum)) 
					i++;
		  		i--;
				m_AzimFreq[i]++;
				...
				i = 0;
				while ((dang >= m_LeftVal+i*m_DangLevel) && (i < m_DangNum))
					i++;
		  		i--;
				m_DangFreq[i]++;
