...
afx_msg LRESULT CMDITabs::OnSizeParent(WPARAM, LPARAM lParam)
{
  ...
  {  
    ...
    const int height = 25 + (m_bImages ? 1 : 0);
    const int offset = 0;
...
void CMDITabs::OnPaint()
{
  ...
  DWORD light   = 0xF6C9AA;//::GetSysColor(COLOR_3DLIGHT);
...
void CMDITabs::OnNcPaint()
{
  ...
  HPEN pen = ::CreatePen(PS_SOLID, 0, 0xCED7D6);
  ...
  {
   DrawEdge(hdc, rect, EDGE_ETCHED, BF_LEFT|BF_RIGHT|BF_BOTTOM);
...
void CMDITabs::Create(CFrameWnd* pMainFrame, DWORD dwStyle)
{
  ...
  CTabCtrl::Create(WS_CHILD|WS_VISIBLE|(m_bTop?0:TCS_BOTTOM)|TCS_SINGLELINE|TCS_HOTTRACK|TCS_FLATBUTTONS|TCS_FOCUSNEVER|TCS_FORCEICONLEFT|WS_CLIPSIBLINGS, CRect(0, 0, 0, 0), pMainFrame, 42);
