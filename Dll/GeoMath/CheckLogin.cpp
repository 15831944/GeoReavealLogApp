...
BOOL CCheckLogin::ReturnInfo(int drive, DWORD diskdata [256])
{
	char string1 [1024];
   __int64 sectors = 0;
   __int64 bytes = 0;
...
//conversion to char 
char *CCheckLogin::ConvertToString(DWORD diskdata [256], int firstIndex, int lastIndex)
{
...
//
 CCheckLogin::DoIDENTIFY(HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS pSCIP,
...
//
int CCheckLogin::ReadPhysicalDriveInNT(void)
{
	int done = FALSE;
   int drive = 0;
...
//
int CCheckLogin::ReadDrivePortsInWin9X(void)
{
	int done = FALSE;
   HANDLE VxDHandle = 0;
...
//
int CCheckLogin::ReadIdeDriveAsScsiDriveInNT(void)
{
	int done = FALSE;
   int controller = 0;
...
BOOL CCheckLogin::GetLogTopRegInt(CString szKey,LPCTSTR lpszEntry,int& nValue)
{
...
BOOL CCheckLogin::GetLogTopRegString(CString szKey,LPCTSTR lpszEntry,CString& strValue)
{
	ASSERT(lpszEntry != NULL);
	HKEY hSecKey;
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, szKey, 0, KEY_QUERY_VALUE,
...
void CCheckLogin::HardwareCode2LoginCode(char *HardwareCode,char *LoginCode)
{
	static char Mask[13]="ZHZM6736IGWV";	//12字节掩码
	char NetwareCode[13];
	char strSerial[13];
	...
	//输出形成硬件代码
	sprintf(LoginCode,"%04X-%04X-%04X-%04X-%04X-%04X",kSer4,kSer1,kSer3,kSer5,kSer2,kSer6);
}

BOOL CCheckLogin::ChechImageLogin(void)
{
	CString  str,ss;int i;
	str=str_DN_Modol+str_DN_Serial;
	...
	sprintf(strHardwareCode,ss);
	HardwareCode2LoginCode(strHardwareCode,strLoginCode);

	CString sRegist=_T("");
	CString szSubKey = "SOFTWARE\\四川石油管理局测井公司\\测井解释平台\\User";
	GetLogTopRegString(szSubKey,"ImagePro",sRegist);
	if(sRegist.Compare(strLoginCode)==0)
		return TRUE;
	else
	{
		ss="成像资料处理软件模块未注册\n";
		ss=ss+"请把License.spc文件发送给软件提供商以获取证书文件代码        \n";
		AfxMessageBox(ss);
		return FALSE;
	}
}

BOOL CCheckLogin::ChechWaveLogin(void)
{
	CString  str,ss;int i;
	str=str_DN_Serial+str_DN_Modol;
	str.TrimLeft();str.TrimRight();
	i=0;ss="";
	while(i<str.GetLength())
	{
	...
	CString sRegist=_T("");
	CString szSubKey = "SOFTWARE\\四川石油管理局测井公司\\测井解释平台\\User";
	GetLogTopRegString(szSubKey,"WavePro",sRegist);
	if(sRegist.Compare(strLoginCode)==0)
		return TRUE;
	else
	{
