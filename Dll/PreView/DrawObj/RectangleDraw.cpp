/******************************************************************
*   RectDrw.cpp - implementation of Rectangle draw object class   *           
*******************************************************************/
...
IMPLEMENT_SERIAL(CRectangleObj, CTextObj, 0)
...
CRectangleObj::CRectangleObj(const CRect& rect,CDrawObj* pObj,CLogPlotDoc* pDocument) 
...
void CRectangleObj::Draw(CView* pView,CDC* pDC)
{
	CRect rect = m_Rect;	
	pDoc->LPtoPP(rect);
	...
	if (m_LogBrush.lbStyle == BS_OWNER)
	{
		if(m_LogBrush.lbHatch >= 0) 
			LithFillBox(rect,(int)m_LogBrush.lbHatch,m_LogPen.lopnColor,rect.TopLeft(),pDC,m_LogBrush.lbColor);
...
int CRectangleObj::HitTest(CView* pView,const CPoint& point)
{
	ASSERT_VALID(this);
	ASSERT(pView != NULL);
...
void CRectangleObj::CreateFocusObj(CView* pView)
{
	m_pFocusObj = new CRectangleObj(m_Rect,m_pParentObj,pDoc);
...
void CRectangleObj::DeleteFocusObj(CView* pView)
{
...
void CRectangleObj::DrawFocusObj(CView* pView)
{
	ASSERT((m_pFocusObj != NULL) && (pView != NULL));
...
void CRectangleObj::DrawFocus(CView* pView)
{
	ASSERT(pView != NULL);
...
void CRectangleObj::OnLButtonDown(CView* pView, UINT nFlags, const CPoint& point)
{
    if (m_bSelected)
    {
		
		if (c_ToolShape == rectangletool)
		{
			m_SelectMode = size;
			nDragHandle = 5;
		}              
...
void CRectangleObj::OnLButtonUp(CView* pView, UINT nFlags, const CPoint& point)
{
	if (bInput) return;
	if (m_SelectMode == move)
	{
...
void CRectangleObj::OnMouseMove(CView* pView, UINT nFlags, const CPoint& point)
{
