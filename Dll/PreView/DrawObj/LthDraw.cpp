...
CLthPoint::CLthPoint(CRect rect, int LithNo,unsigned long ColorNo, int LithWidth,CLogPlotDoc* pDocument)
{
	pDoc = pDocument;
	m_LithNo = LithNo;
	m_ColorNo=ColorNo;
...
void CLthPoint::Draw(CDC* pDC,COLORREF LithColor)
{
	...
	{
		if(m_LithWidth==0||m_LithWidth==2)
			LithFillBox(rect,m_LithNo,LithColor,rect.TopLeft(),pDC,m_ColorNo);
		else
			LithFillBox(rect,1,m_ColorNo,rect.TopLeft(),pDC,m_ColorNo);
	}
}
...
CLthObj::CLthObj(CString strPathName, const CRect& rect,CDrawObj* pParentObj,CLogPlotDoc* pDoc)
			...
			while (File.ReadString(buffer, 1024)) 
			{
				...
				*end = _T('\0');
					...
					&&(m_LthNo>=0))
				{
					...
					if(m_LithWidth==2)
					{
						int xx1=rect.Width()/12;
						int xx=(m_LthNo)%11;
						rect.right =rect.left+xx1+xx*xx1;
					}
...
BOOL CLthObj::Properties(CView* pView)
{
	...
	m_ResultPage.m_GroupCaption="标识";
	m_ResultPage.m_LeftCaption="符号";
	m_ResultPage.m_RightCaption="颜色";
