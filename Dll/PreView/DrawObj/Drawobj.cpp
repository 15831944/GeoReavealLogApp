...
CDrawObj::CDrawObj(const CRect& rect, CDrawObj* pParentObj,CLogPlotDoc* pDocument)
{
	m_bSelected = FALSE;
	bEnableChildSizeMessage = TRUE;
	m_Rect = rect;
...
void CDrawObj::DrawTracker(CView* pView, CDC* pDC, enum TrackerState state)
{
	ASSERT_VALID(this);
...
void CDrawObj::Draw(CView* pView,CDC* pDC)
{
	...
	while (pos != NULL)
	{
 		...
 		if (pObj != NULL)
		{
			if(pDoc->m_bRunFirst)
			{
				pDoc->m_pGuage->SetPos(pDoc->m_DrawNo);
				pDoc->m_DrawNo++;
			}
			pObj->Draw(pView,pDC);
		}
...
{
	CRect rect1;
	CClientDC dc(pDoc->m_pView);              
	/*pDoc->m_pView->OnPrepareDC(&dc);*/
	rect1 = rect;
	pDoc->LPtoPP(&rect1);
	dc.LPtoDP(rect1);
	rect1.InflateRect(3,3);
	pDoc->m_pView->InvalidateRect(rect1);
	pDoc->m_pView->UpdateWindow();
}