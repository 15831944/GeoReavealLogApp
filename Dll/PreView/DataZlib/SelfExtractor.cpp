...
int CSelfExtractor::Create(CString Filename, funcPtr pFn /* = NULL */, void* userData /*=NULL*/)
{
	//Make sure we have some files to add
	if(m_nFiles < 1)
		return NOTHING_TO_DO;
...
int CSelfExtractor::CreateArchive(CFile* pFile, funcPtr pFn, void* userData)
{
	...
	try		
	{
		...
		pFile->Write(buffer, strlen(SFX_VERSION));
...
* 
* Parameters: 
*    CFile* pFile - the file which is being read
* 
* Return: 
*    int: amount of data read this time
*******************************************************************************/
int CSelfExtractor::ZLibLoadInput(CFile* pFile)
{
    if(avail_in == 0) 
	{
        next_in = m_ZLibInputBuffer;
        avail_in = pFile->Read(m_ZLibInputBuffer, ZLibInputLength);
    }
...
* 
* Parameters: 
*    CFile* pFile - the output file
* 
* Return: 
*    int: amount of data written this time
*******************************************************************************/
int CSelfExtractor::ZLibFlushOutput(CFile* pFile)
{
    unsigned int count = ZLibOutputLength - avail_out;
    if(count) 
	{
        pFile->Write(m_ZLibOutputBuffer, count);
        next_out = m_ZLibOutputBuffer;
        avail_out = ZLibOutputLength;
    }
