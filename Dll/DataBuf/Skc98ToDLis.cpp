...
bool CSkc98ToDLis::Transform()
{
	...
	////////////////////////////////////////////
	GetDlisFDataIndexFromSkc98(); //对SKC98通道信息进行分组
	Prt_FdataItemInfo(); //打印帧分组信息
	SetDlisChannelsFromSkc98(); //根据SKC98信息设置通道信息
	SetDlisFrameFromSkc98(); //设置DLIS帧对象属性
	SetAxisFromSkc98(); //设置DLIS轴对象属性
...
void CSkc98ToDLis::SetDlisChannelsFromSkc98()
{
	...
	for(int i=0;i<CurveIndex.GetSize();i++)
	{
		...
		else
		{
			...
			DlisWrite->Channel[i].Dimension[0]=Skc98R->Skc98Head.CurveTypes[index]/2;//维元素
			DlisWrite->Channel[i].Dimension[1]=0; //维元素
			DlisWrite->Channel[i].Dimension[2]=0; //维元素
		}
	}
	...
	for(int i=0;i<NumItem;i++)
	{
		...
		for(int k=0;k<CurveIndex.GetSize();k++)
		{
			if(ItemNum[k]==i)
			{
				...
				{
					DlisWrite->Frame[i].Index_Start=Skc98R->Skc98Head.Stdep; //索引最小值
					DlisWrite->Frame[i].Index_Stop=Skc98R->Skc98Head.Endep; //索引最大值
				}
				else
				{
					DlisWrite->Frame[i].Index_Start=Skc98R->Skc98Head.Endep; //索引最小值
					DlisWrite->Frame[i].Index_Stop=Skc98R->Skc98Head.Stdep; //索引最大值
				}
				DlisWrite->Frame[i].Index_Step=float(fabs(Skc98R->yStep[index])); //步长
...
void CSkc98ToDLis::SetAxisFromSkc98()
{
	...
	{
		int index=CurveIndex.GetAt(i); //SKC98曲线索引号
		int NumDim=1;
		if(!Skc98R->IsConvCurve(index))
		{
			NumDim=2;
...
void CSkc98ToDLis::SetStoUnitLab()
{
	...
	sprintf(str,"%-60.60s","SKC98 to DLIS Conversion"); //存储单标志
...
void CSkc98ToDLis::SetFileHeader()
{
	...
	sprintf(DlisWrite->FileHeader.ID,"%-65.65s","SKC98.001");//ID号
}
...
void CSkc98ToDLis::Skc98DataToDlis()
{
	...
	for(int nItem=0;nItem<NumItem;nItem++)
	{
		...
		for(int i=0;i<NumCurve;i++)
		{
			...
			int IndexSkc98=CurveIndex.GetAt(index); //曲线在SKC98中索引
