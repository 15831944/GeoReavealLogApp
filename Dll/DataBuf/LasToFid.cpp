// LasToFid.cpp: implementation of the CLasToFid class.
//
//////////////////////////////////////////////////////////////////////
...
bool CLasToFid::LasToFid(CString SourceFile,CString sObjectPath,int nCurvesRadio,FILE *fo)
{
	CString s;
...
bool CLasToFid::Transform()
{
	CString s;
	if(!IsFwdLas(m_SourceFile))
	{
		fprintf(fp,"%s is not Las Format\n\n",m_SourceFile);
		return false;
	}
	...
	if(!FwdLasR->Open(m_SourceFile)) 
	{
		fprintf(fp,"\n!!!!链接的数据文件打开失败\n\n");
	}
	else
	{
		FwdLasR->ReadHead(); //读Las头
		FwdLasR->Prt_Header(fp);
		SelectCurves();
		if(CurveIndex.GetSize()==0) return false;
		SetFidHead();
		FidWrite->WriteFidInfo();
		int NumPoint=FwdLasR->FwdLasHd.NumPoint;
		int NumLog=FwdLasR->FwdLasHd.NumLog;
		...
		FwdLasR->ReadCurve(buf);//读全部数据
		FwdLasR->Close(); //关闭输入文件
...
void CLasToFid::DataNormalization(int NumPoint,int NumLog,float *buf)
{
	//如果为上测，则将数据重新排序
	if(FwdLasR->FwdLasHd.Rlev>0) return;
...
void CLasToFid::SelectCurves()
{
	CString s;
	bool *CurveID=new bool[FwdLasR->FwdLasHd.NumLog];
	for(int i=0;i<FwdLasR->FwdLasHd.NumLog;i++)
		CurveID[i]=false;
	CString sCurveName;
	switch(m_nCurvesRadio)
	{
	case 0://曲线过滤		
		for(i=0;i<FwdLasR->FwdLasHd.NumLog;i++)
		{
			bool bOk=false;
			sCurveName.Format ("%s",FwdLasR->FwdLasHd.CurveName[i]);
	...
	{
		for(int j=0;j<FwdLasR->FwdLasHd.NumLog;j++)
		{
			s.Format ("%s",FwdLasR->FwdLasHd.CurveName[j]);
...
void CLasToFid::SetFidHead()
{
	...
	for(int i=0;i<CurveIndex.GetSize();i++)
	{
		...
		if(FwdLasR->FwdLasHd.Rlev>0)
		{
			FidWrite->FidChannel[i].DimInfo[0].Start=FwdLasR->FwdLasHd.Stdep;
			FidWrite->FidChannel[i].DimInfo[0].Stop=FwdLasR->FwdLasHd.Endep;
			FidWrite->FidChannel[i].DimInfo[0].Rlev=FwdLasR->FwdLasHd.Rlev;
		}
		else
		{
			FidWrite->FidChannel[i].DimInfo[0].Start=FwdLasR->FwdLasHd.Endep;
			FidWrite->FidChannel[i].DimInfo[0].Stop=FwdLasR->FwdLasHd.Stdep;
			FidWrite->FidChannel[i].DimInfo[0].Rlev=float(fabs(FwdLasR->FwdLasHd.Rlev));
		}
