// Qh716ToFid.cpp: implementation of the CQh716ToFid class.
//
//////////////////////////////////////////////////////////////////////
...
bool CQh716ToFid::Qh716ToFid(CString SourceFile,CString sObjectPath,int nCurvesRadio,FILE *fo)
{
	CString s;
...
bool CQh716ToFid::Transform()
{
	CString s;
	if(!IsQH716(m_SourceFile))
	{
		fprintf(fp,"%s is not QH716 Format\n\n",m_SourceFile);
		return false;
	}
	...
	if(!Qh716R->Open(m_SourceFile)) 
	{
		fprintf(fp,"\n!!!!链结的数据文件打开失败!\n\n");
	}
	else
	{
		Qh716R->ReadHead(); //读QH716头
		Qh716R->Prt_Header(fp);
		SelectCurves();
		SetFidHead();
		FidWrite->WriteFidInfo();
		int NumPoint=Qh716R->NumPoint;
		float *buf=new float[Qh716R->NumBlock*int(Qh716R->Head.Feet)];
		...
		for(int i=0;i<FidWrite->FidHead.NumLog;i++)
		{
			...
			Qh716R->ReadCurve(index,buf);//读一条曲线数据
...
void CQh716ToFid::DataNormalization(int NumPoint,int NumLog,float *buf)
{
	//如果为上测，则将数据重新排序
	if(Qh716R->Head.Rlev>0) return;
...
void CQh716ToFid::SelectCurves()
{
	CString s;
	bool *CurveID=new bool[Qh716R->Head.NumLog];
	for(int i=0;i<Qh716R->Head.NumLog;i++)
	{
		CurveID[i]=false;
	}
	CString sCurveName;
	...
	{
	case 0://曲线过滤	
		for(i=0;i<Qh716R->Head.NumLog;i++)
		{
			bool bOk=false;
			sCurveName.Format ("%-.4s",Qh716R->Head.CurveName[i]);
	...
	{
		for(int j=0;j<Qh716R->Head.NumLog;j++)
		{
			s.Format ("%-.4s",Qh716R->Head.CurveName[j]);
...
void CQh716ToFid::SetFidHead()
{
	...
	for(int i=0;i<CurveIndex.GetSize();i++)
	{
		...
		if(Qh716R->Head.Rlev>0)
		{
			FidWrite->FidChannel[i].DimInfo[0].Start=Qh716R->Head.Stdep;
			FidWrite->FidChannel[i].DimInfo[0].Stop=Qh716R->Head.Endep;
			FidWrite->FidChannel[i].DimInfo[0].Rlev=Qh716R->Head.Rlev;
		}
		else
		{
			FidWrite->FidChannel[i].DimInfo[0].Start=Qh716R->Head.Endep;
			FidWrite->FidChannel[i].DimInfo[0].Stop=Qh716R->Head.Stdep;
			FidWrite->FidChannel[i].DimInfo[0].Rlev=float(fabs(Qh716R->Head.Rlev));
		}
