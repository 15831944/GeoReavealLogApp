...
void LithFillPoly(LPPOINT point, int nPointNum,int nLthNo,COLORREF nColor,
				  CPoint org,CDC* pDC1,COLORREF nBkColor)
{	
	...
	if (nLthNo == 1)
	{
		...
		CBrush* pOldBrush = pDC1->SelectObject(&br);
		pDC1->Polygon(point,nPointNum);
		pDC1->SelectObject(pOldBrush);
		return;
	}
	CDC dc;
	dc.Attach(pDC1->m_hDC);
	CDC* pDC=&dc;
...
//	read Lith Color data
	nLthNo--;
	short LithColor[32][32],m,n;
	char s1[32],s2[10];
	strcpy(s1,"Lith");        
	ltoa(nLthNo,s2,10);
	strcat(s1,s2); 
	strcat(s1,".dat"); 
	CString PathStr=GetWorkPath("LithLib")+s1;
	FILE* fp;
	if(fp=fopen(PathStr,"rb")) 
	{
		fread(LithColor,2048,1,fp);
		fclose(fp);
	}
	else
	{fclose(fp);return;}
				 ...
				 CPoint org,CDC* pDC1,COLORREF nBkColor)
{
	...
	{				
		CBrush br(nColor);
		pDC1->FillRect(CRect(xl,yt,xr,yb),&br);
		return;
	}
	CDC dc;
	dc.Attach(pDC1->m_hDC);
				   ...
				   CPoint org, CDC* pDC1,COLORREF nBkColor)
{
	if (nLthNo == 1)
	{
		CBrush br(nColor);
		CBrush* pOldBrush = pDC1->SelectObject(&br);
		pDC1->RoundRect(xl,yt,xr,yb,roundNess.x,roundNess.y);
		pDC1->SelectObject(pOldBrush);
		return;
	}
	CDC dc;
...
void CFillPatternBox::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (lpDIS->itemAction & ODA_DRAWENTIRE)
	{
		...
		if (style < 0) return;
		ltoa(style,s,10);
		CSize size = pDC->GetTextExtent(s,strlen(s));
		int k = (16 - size.cy)/2;
		CRect rect1 = rect;
		rect1.top += k; rect1.bottom -= k;
   		pDC->TextOut(rect1.left,rect1.top,s,strlen(s));
