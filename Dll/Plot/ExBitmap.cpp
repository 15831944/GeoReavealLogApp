// EnBitmap.cpp: implementation of the CExBitmap class (c) daniel godson 2002.
//
...
CExBitmap::CExBitmap(COLORREF crBkgnd) : m_crBkgnd(crBkgnd)
{
...
BOOL CExBitmap::LoadImage(UINT uIDRes, LPCTSTR szResourceType, HMODULE hInst, COLORREF crBack)
{
...
BOOL CExBitmap::LoadImage(LPCTSTR szImagePath, COLORREF crBack)
{
	ASSERT(m_hObject == NULL);      // only attach once, detach on destroy
...
HBITMAP CExBitmap::LoadImageFile(LPCTSTR szImagePath, COLORREF crBack)
{
...
HBITMAP CExBitmap::LoadImageResource(UINT uIDRes, LPCTSTR szResourceType, HMODULE hInst, COLORREF crBack)
{
	BYTE* pBuff = NULL;
	int nSize = 0;
	HBITMAP hbm = NULL;
...
IPicture* CExBitmap::LoadFromBuffer(BYTE* pBuff, int nSize)
{
	bool bResult = false;
...
BOOL CExBitmap::GetResource(LPCTSTR lpName, LPCTSTR lpType, HMODULE hInst, void* pResource, int& nBufSize)
{ 
	HRSRC		hResInfo;
	HANDLE		hRes;
	LPSTR		lpRes	= NULL; 
...
HBITMAP CExBitmap::ExtractBitmap(IPicture* pPicture, COLORREF crBack)
{
	ASSERT(pPicture);
...
int CExBitmap::GetFileType(LPCTSTR szImagePath)
{
...
BOOL CExBitmap::RotateImage(int nDegrees, BOOL bEnableWeighting)
{
	return ProcessImage(&CImageRotator(nDegrees, bEnableWeighting));
}

BOOL CExBitmap::ShearImage(int nHorz, int nVert, BOOL bEnableWeighting)
{
	return ProcessImage(&CImageShearer(nHorz, nVert, bEnableWeighting));
}

BOOL CExBitmap::GrayImage()
{
	return ProcessImage(&CImageGrayer());
}

BOOL CExBitmap::BlurImage(int nAmount)
{
	return ProcessImage(&CImageBlurrer(nAmount));
}

BOOL CExBitmap::SharpenImage(int nAmount)
{
	return ProcessImage(&CImageSharpener(nAmount));
}

BOOL CExBitmap::ResizeImage(double dFactor)
{
	return ProcessImage(&CImageResizer(dFactor));
}

BOOL CExBitmap::FlipImage(BOOL bHorz, BOOL bVert)
{
	return ProcessImage(&CImageFlipper(bHorz, bVert));
}

BOOL CExBitmap::NegateImage()
{
	return ProcessImage(&CImageNegator());
}

BOOL CExBitmap::ReplaceColor(COLORREF crFrom, COLORREF crTo)
{
	return ProcessImage(&CColorReplacer(crFrom, crTo));
}

BOOL CExBitmap::ProcessImage(C32BitImageProcessor* pProcessor)
{
...
BOOL CExBitmap::ProcessImage(C32BIPArray& aProcessors)
{
...
BOOL CExBitmap::PrepareBitmapInfo32(BITMAPINFO& bi, HBITMAP hBitmap)
{
	if (!hBitmap)
...
BOOL CExBitmap::CopyImage(HBITMAP hBitmap)
{
	ASSERT (hBitmap);
...
BOOL CExBitmap::CopyImage(CBitmap* pBitmap)
{
	if (!pBitmap)
		return FALSE;
...
BOOL CExBitmap::Fill(RGBX* pPixels, CSize size, COLORREF color)
{
	if (!pPixels)
		return FALSE;
